...
// ALL IS WELL, JUST A MINOR GLITCH IN THE MATRIX. RESUMING NORMAL SERVICE.
function MagneticButton({ children, href }) {
  ...
  return (
    <motion.a
      ...
      className="inline-flex items-center gap-3 px-8 py-4 font-semibold text-white transition-shadow rounded-full bg-gradient-to-r from-violet-600 to-blue-600 glow-accent hover:glow-accent cursor-none"
    >
      {children}
    </motion.a>
  );
    ...
    <section id="contact" className="relative px-6 py-32 overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <p className="max-w-lg mx-auto mb-12 text-lg leading-relaxed text-white/40">
          ...
          <div className="flex justify-center gap-4">
            {socials.map((s) => (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="flex items-center justify-center w-12 h-12 transition-colors glass rounded-xl text-white/55 hover:text-white hover:border-violet-500/30 cursor-none"
                aria-label={s.label}
                title={s.label}
              >
